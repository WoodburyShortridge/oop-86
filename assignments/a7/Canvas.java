...
public class Canvas extends JPanel implements MouseListener {
        ...
        if (model.getArrayBool("removedList").get(id) == false ) {
          ...
          if (model.getArrayString("satelliteType").get(id) == "Research") {
            new ResearchSatellite(
              zoomedPosX((int)model.getArrayPoint("satellitePos").get(id).getX()),
              zoomedPosY((int)model.getArrayPoint("satellitePos").get(id).getY()),
              id,
  ...
  public void mousePressed (MouseEvent event) {
      ...
      for(int id : model.getArrayInt("satelliteList")) {
        if (model.getArrayBool("removedList").get(id) == false) {
          ...
          if (Math.abs(event.getPoint().x - zoomedPosX(p.x)) < 50) {
            if (Math.abs(event.getPoint().y - zoomedPosY(p.y)) < 50) {
  ...
  private int zoomedPosX (int x) {
    double zoom = model.getZoom() / 10;
    int newX = x;
